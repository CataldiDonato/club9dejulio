            ...
            >
                ...
                <div className="bg-club-dark text-white p-4 flex justify-between items-center border-b border-gray-700 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-yellow-400 font-bold text-xs tracking-wider uppercase">CAMPEÓN</span>
                                ...
                                <div className="md:col-span-2 space-y-4">
                                    {championship.image ? (
                                        <div className="rounded-lg overflow-hidden shadow-md border-4 border-white">
                                            ...
                                            <p className="text-[10px] text-center text-gray-400 italic p-3 bg-gray-50 leading-relaxed">
                                                {championship.photoCaption || `Foto histórica del plantel ${championship.year}`}
                                            </p>
                                        </div>
                                    ) : (
                                    ...
                                    {championship.stats && (
                                        <div className="bg-blue-900 text-white p-4 rounded-xl shadow-lg">
                                            <h4 className="font-black text-center mb-3 text-yellow-500 uppercase tracking-widest text-xs">Números Finales</h4>
                                            <div className="grid grid-cols-2 gap-2 text-center">
                                                <div className="bg-white/10 p-2 rounded">
                                                    <p className="text-2xl font-black">{championship.stats.pj || '-'}</p>
                                                    <p className="text-[10px] uppercase">Partidos</p>
                                                </div>
                                                <div className="bg-white/10 p-2 rounded">
                                                    <p className="text-2xl font-black">{championship.stats.gf || '-'}</p>
                                                ...
                                                {championship.stats.pg !== undefined && (
                                                    <div className="bg-white/10 p-2 rounded col-span-2 flex justify-around items-center">
                                                        <div>
                                                            <p className="text-lg font-bold text-green-400">{championship.stats.pg}</p>
                                                            <p className="text-[8px] uppercase">Ganados</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-lg font-bold text-yellow-400">{championship.stats.pe}</p>
                                                            <p className="text-[8px] uppercase">Empatados</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-lg font-bold text-red-400">{championship.stats.pp}</p>
                                ...
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-gray-50 text-gray-600 font-bold">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 hidden md:table-cell">Goleadores</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {championship.campaign.map((match, idx) => (
                                            <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                                                <td className="px-4 py-3 text-gray-500 text-xs">{match.date || '-'}</td>
                                                <td className="px-4 py-3 font-semibold text-gray-800 uppercase tracking-tight">{match.rival}</td>
                                                <td className="px-4 py-3 text-center">
                                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${match.cond === 'L' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                                                        {match.cond === 'L' ? 'Local' : match.cond === 'V' ? 'Visit.' : 'Neutro'}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-center font-black text-club-blue text-base">
                                                    {match.result}
                                                </td>
                                                <td className="px-4 py-3 hidden md:table-cell text-xs italic text-gray-600">
                                                    {match.scorers || '-'}
                                                </td>
                                            </tr>
                                        ))}
                        ...
                        <div className="max-w-3xl mx-auto animate-fadeIn">
                            ...
                            <div className="grid sm:grid-cols-2 gap-x-12 gap-y-2">
                                {Array.isArray(championship.squad) ? (
                                    championship.squad.map((player, idx) => (
                                        <div key={idx} className="flex justify-between items-center py-2 border-b border-gray-50 group hover:bg-gray-50 px-2 rounded transition-colors">
                                            <div className="flex items-center gap-3">
                                                {player.number && (
                                                ...
                                                <div>
                                                    ...
                                                    {player.pos && (
                                                        <p className="text-[10px] text-gray-400 font-semibold uppercase">{player.pos}</p>
                                                    )}
                                                </div>
                                            </div>
                                            {player.goals && (
                                                <div className="flex items-center gap-1.5 bg-yellow-100 px-2 py-0.5 rounded-full">
                                                    ...
                                                    <span className="text-xs font-black text-yellow-800">{player.goals}</span>
                                                </div>
                                            )}
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-gray-500 italic col-span-2">{championship.squad}</p>
                                )}
                            </div>
                            ...
                            <div className="mt-12 p-6 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-center">
                                <p className="text-sm text-gray-400 font-medium">
                                    "La gloria es eterna, el plantel de {championship.year} vivirá por siempre en el corazón lanudo."
                                </p>
                            </div>
                        </div>
                    )}
